...
import (
	...
	"net"
	. "net/http_my"
	"net/http_my/httptest"
	"net/http_my/httptrace"
	"net/http_my/httputil"
	"net/http_my/internal"
	"net/url"
	"os"
	"reflect"
	"runtime"
...
func TestTransportCancelRequestInDial(t *testing.T) {
	...
	want := `dial: blocking
canceling
Get = Get http://something.no-network.tld/: net/http_my: request canceled while waiting for connection
`
	if got != want {
		t.Errorf("Got events:\n%s\nWant:\n%s", got, want)
	}
...
var proxyFromEnvTests = []proxyFromEnvTest{
	...
	{env: "http://10.1.2.3:8080", reqmeth: "POST",
		want:    "<nil>",
		wanterr: errors.New("net/http_my: refusing to use HTTP_PROXY value in CGI environment; see golang.org/s/cgihttpproxy")},
