...
// is preferred:
//    https://golang.org/pkg/net/http_my/#ResponseWriter
//    https://golang.org/pkg/net/http_my/#example_ResponseWriter_trailers
...
func (cr *connReader) backgroundRead() {
	...
	if ne, ok := err.(net.Error); ok && cr.aborted && ne.Timeout() {
		...
		log.Println("*connReader) backgroundRead problem")
...
func (cr *connReader) closeNotifyFromPipelinedRequest() {
	log.Println("closeNotifyFromPipelinedRequest")
	cr.closeNotify()
}
...
var ErrAbortHandler = errors.New("net/http_my: abort Handler")
...
func (c *conn) serve(ctx context.Context) {
	...
	for {
		...
		if requestBodyRemains(req.Body) {
			log.Println("requestBodyRemains triggers registerOnHitEOF + startBackgroundRead")
			registerOnHitEOF(req.Body, w.conn.r.startBackgroundRead)
		} else {
			if w.conn.bufr.Buffered() > 0 {
				w.conn.r.closeNotifyFromPipelinedRequest()
			}
			log.Println("else triggers just startBackgroundRead")
...
func (w *response) Hijack() (rwc net.Conn, buf *bufio.ReadWriter, err error) {
	if w.handlerDone.isSet() {
		panic("net/http_my: Hijack called after ServeHTTP finished")
	}
	if w.wroteHeader {
		w.cw.flush()
	}
...
func (w *response) CloseNotify() <-chan bool {
	if w.handlerDone.isSet() {
		panic("net/http_my: CloseNotify called after ServeHTTP finished")
	}
