...
func ProxyFromEnvironment(req *Request) (*url.URL, error) {
	...
	if proxy == "" {
		...
		if proxy != "" && os.Getenv("REQUEST_METHOD") != "" {
			return nil, errors.New("net/http_my: refusing to use HTTP_PROXY value in CGI environment; see golang.org/s/cgihttpproxy")
		}
	}
	if proxy == "" {
		return nil, nil
...
func (t *Transport) RoundTrip(req *Request) (*Response, error) {
	...
	if isHTTP {
		for k, vv := range req.Header {
			if !httplex.ValidHeaderFieldName(k) {
				return nil, fmt.Errorf("net/http_my: invalid header field name %q", k)
			}
			for _, v := range vv {
				if !httplex.ValidHeaderFieldValue(v) {
					return nil, fmt.Errorf("net/http_my: invalid header field value %q for key %v", v, k)
				}
			}
		}
	}
	...
	if req.Method != "" && !validMethod(req.Method) {
		return nil, fmt.Errorf("net/http_my: invalid method %q", req.Method)
	}
	if req.URL.Host == "" {
		req.closeBody()
...
var ErrSkipAltProtocol = errors.New("net/http_my: skip alternate protocol")
...
func (e transportReadFromServerError) Error() string {
	return fmt.Sprintf("net/http_my: Transport failed to read from server: %v", e.err)
}
...
func (t *Transport) dial(ctx context.Context, network, addr string) (net.Conn, error) {
	...
	if t.Dial != nil {
		...
		if c == nil && err == nil {
			err = errors.New("net/http_my: Transport.Dial hook returned (nil, nil)")
		}
		return c, err
	}
...
func (t *Transport) dialConn(ctx context.Context, cm connectMethod) (*persistConn, error) {
	...
	if tlsDial {
		...
		if pconn.conn == nil {
			return nil, errors.New("net/http_my: Transport.DialTLS returned (nil, nil)")
		}
...
func (pc *persistConn) mapRoundTripErrorAfterClosed(req *Request, startBytesWritten int64) error {
	...
	return fmt.Errorf("net/http_my: HTTP/1.x transport connection broken: %v", err)
...
func (pc *persistConn) readLoop() {
	...
	for alive {
		...
		if err != nil {
			if pc.readLimit <= 0 {
				err = fmt.Errorf("net/http_my: server response headers exceeded %d bytes; aborted", pc.maxHeaderResponseSize())
			}
...
var errTimeout error = &httpError{err: "net/http_my: timeout awaiting response headers", timeout: true}
var errRequestCanceled = errors.New("net/http_my: request canceled")
var errRequestCanceledConn = errors.New("net/http_my: request canceled while waiting for connection") // TODO: unify?
...
func (tlsHandshakeTimeoutError) Error() string   { return "net/http_my: TLS handshake timeout" }
